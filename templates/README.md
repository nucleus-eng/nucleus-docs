...
**Processes** wrap lab protocols: the step-by-step instructions someone follows at the bench. A Process also includes an overview, materials, performance data from executing the protocol, and links to relevant DevNotes. Processes are self-contained. They describe how to make or prepare something (grow bacteria, purify a protein, assemble cytosol) without assuming which modules or system the output ends up in.
...
If the data describes an intrinsic property of a component (something you'd find in a database), it goes in the Module spec. If the data comes from executing a protocol, it goes in the Process. If the data comes from running specific modules in a specific context, it goes in an Implementation.