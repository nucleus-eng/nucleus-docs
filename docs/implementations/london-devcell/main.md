---
title: "London DevCell: AHSL Colorimetric Reporter"
...
The London DevCell detects a bacterial quorum-sensing signal and reports it as a visible color change from inside a hydrogel. Synthetic cells built on [S30 Lysate](../../modules/s30-lysate/spec.md) carry a LuxR/pLux circuit; AHSL diffusing in from an external bacterial source drives expression of phospholipase A1 (PLA1), which lyses the cell and its neighboring [Substrate SUV: CPRG](../../modules/substrate-cprg-suv/spec.md), releasing CPRG to LacZ and turning the gel purple.
...
**Polymersomes are out.** London is not pursuing diblock-copolymer polymersomes. The device is lipid-based throughout. This invalidates [`london-quorum-sensing-polymersome`](https://devnotes.nucleus.engineering/articles/019b13f8-9a25-7553-a88d-fa4f19790d13) as a source document, which leaves the [AHSL Detector](../../modules/detector-3oc6-hsl/spec.md) with no backing document at all.
...
**The readout is leaky.** Discernible but incomplete separation between +AHSL and −AHSL, with DNA and AHSL concentrations still being optimized. Exterior LacZ leakage is a known contributor, and its mitigation ([Degrade Exterior LacZ](../../processes/degrade-exterior-lacz/main.md)) has never been run.
...
| Sensing | [AHSL Detector](../../modules/detector-3oc6-hsl/spec.md) → [AHSL Sensing Cell](../../modules/ahsl-sensing-cell/spec.md) | characterized in lysate; leaky |
...
:::{note} This device uses a different AHSL sensor from the published Nucleus one
The distribution's existing AHSL work is the [IV-HSL Emitter](../../modules/emitter-ivhsl/spec.md): BjaI makes a branched AHSL and a co-cultured *E. coli* strain carrying `bjaR-GFP-native` does the sensing. This device instead senses 3OC6-HSL with LuxR/pLux, inside a synthetic cell. Different analyte, different receptor, different host for the sensing step. Both are AHSL sensors; they are not interchangeable.
:::
...
3. **A synthetic cell + SUV + AHSL result has not been reproduced.** Negative controls in that test turned purple, attributed to leaky old-stock liposomes. Reproducing it is an open action item.
4. **Source-document arithmetic is unresolved.** The AHSL reaction table in `Demo Status - London.docx` does not reconcile — the AHSL row is off by 50× — and the plasmid dose appears as both 37 and 80 ng/µL.
:::
